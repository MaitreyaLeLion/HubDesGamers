...
var Bone = class _Bone extends Node {
   ...
   */
  constructor(name44, skeleton, parentBone = null, localMatrix = null, restMatrix = null, bindMatrix = null, index = null) {
    super(name44, skeleton.getScene());
    this.name = name44;
    this.children = [];
    this.animations = [];
    this._index = null;
...
Mesh._instancedMeshFactory = (name44, mesh) => {
  const instance = new InstancedMesh(name44, mesh);
...
var InstancedMesh = class extends AbstractMesh {
   ...
   */
  constructor(name44, source) {
    super(name44, source.getScene());
   ...
   */
  createInstance(name44) {
    return this._sourceMesh.createInstance(name44);
  }
  /**
   ...
   * @returns the clone
   */
  clone(name44, newParent = null, doNotCloneChildren, newSourceMesh) {
    const result = (newSourceMesh || this._sourceMesh).createInstance(name44);
...
var AssetContainer = class extends AbstractScene {
  ...
  mergeAnimationsTo(scene = EngineStore.LastCreatedScene, animatables, targetConverter = null) {
    ...
    const _targetConverter = targetConverter ? targetConverter : (target) => {
      ...
      const name44 = target.name.split(".").join("").split("_primitive")[0];
      switch (targetProperty) {
        case "position":
        case "rotationQuaternion":
          node = scene.getTransformNodeByName(target.name) || scene.getTransformNodeByName(name44);
          break;
        case "influence":
          node = scene.getMorphTargetByName(target.name) || scene.getMorphTargetByName(name44);
          break;
        default:
          node = scene.getNodeByName(target.name) || scene.getNodeByName(name44);
      }
      return node;
    };
...
var Sound = class _Sound {
   ...
   */
  constructor(name44, urlOrArrayBuffer, scene, readyToPlayCallback = null, options) {
    var _a;
    this.autoplay = false;
    this._loop = false;
...
var Skeleton = class _Skeleton {
  ...
  toString(fullDetails) {
    ...
    if (fullDetails) {
      ret += ", Ranges: {";
      let first = true;
      for (const name44 in this._ranges) {
        if (first) {
          ret += ", ";
          first = false;
        }
   ...
   */
  getBoneIndexByName(name44) {
    for (let boneIndex = 0, cache = this.bones.length; boneIndex < cache; boneIndex++) {
      if (this.bones[boneIndex].name === name44) {
        return boneIndex;
      }
    }
    return -1;
   ...
   */
  createAnimationRange(name44, from, to) {
    if (!this._ranges[name44]) {
      this._ranges[name44] = new AnimationRange(name44, from, to);
      for (let i = 0, nBones = this.bones.length; i < nBones; i++) {
        if (this.bones[i].animations[0]) {
          this.bones[i].animations[0].createRange(name44, from, to);
        }
      }
    }
  }
   ...
   */
  deleteAnimationRange(name44, deleteFrames = true) {
    for (let i = 0, nBones = this.bones.length; i < nBones; i++) {
      if (this.bones[i].animations[0]) {
        this.bones[i].animations[0].deleteRange(name44, deleteFrames);
      }
    }
    this._ranges[name44] = null;
  }
  /**
   ...
   */
  getAnimationRange(name44) {
    return this._ranges[name44] || null;
  }
  /**
  ...
  getAnimationRanges() {
    ...
    let name44;
    for (name44 in this._ranges) {
      animationRanges.push(this._ranges[name44]);
    }
   ...
   */
  copyAnimationRange(source, name44, rescaleAsRequired = false) {
    if (this._ranges[name44] || !source.getAnimationRange(name44)) {
      return false;
    }
    let ret = true;
    ...
    for (i = 0, nBones = this.bones.length; i < nBones; i++) {
      ...
      if (sourceBone) {
        ret = ret && this.bones[i].copyAnimationRange(sourceBone, name44, frameOffset, rescaleAsRequired, skelDimensionsRatio);
      } else {
        Logger.Warn("copyAnimationRange: not same rig, missing source bone " + boneName);
        ret = false;
      }
    }
    const range = source.getAnimationRange(name44);
    if (range) {
      this._ranges[name44] = new AnimationRange(name44, range.from + frameOffset, range.to + frameOffset);
    }
    return ret;
  }
  /**
   ...
   */
  beginAnimation(name44, loop, speedRatio, onAnimationEnd) {
    const range = this.getAnimationRange(name44);
    if (!range) {
      return null;
    }
   ...
   */
  clone(name44, id) {
    const result = new _Skeleton(name44, id || name44, this._scene);
  ...
  serialize() {
    ...
    for (let index = 0; index < this.bones.length; index++) {
      ...
      for (const name44 in this._ranges) {
        const source = this._ranges[name44];
        if (!source) {
          continue;
        }
        const range = {};
...
Node.AddNodeConstructor("TargetCamera", (name44, scene) => {
  return () => new TargetCamera(name44, Vector3.Zero(), scene);
});
var TargetCamera = class _TargetCamera extends Camera {
   ...
   */
  constructor(name44, position, scene, setActiveOnSceneIfNoneActive = true) {
    super(name44, position, scene, setActiveOnSceneIfNoneActive);
  ...
  createRigCamera(name44, cameraIndex) {
    if (this.cameraRigMode !== Camera.RIG_MODE_NONE) {
      const rigCamera = new _TargetCamera(name44, this.position.clone(), this.getScene());
...
var PassPostProcess = class _PassPostProcess extends PostProcess {
   ...
   */
  constructor(name44, options, camera = null, samplingMode, engine, reusable, textureType = 0, blockCompilation = false) {
    super(name44, "pass", null, null, options, camera, samplingMode, engine, reusable, void 0, textureType, void 0, null, blockCompilation);
  }
  /**
   * @internal
   */
...
var PassCubePostProcess = class _PassCubePostProcess extends PostProcess {
   ...
   */
  constructor(name44, options, camera = null, samplingMode, engine, reusable, textureType = 0, blockCompilation = false) {
    super(name44, "passCube", null, null, options, camera, samplingMode, engine, reusable, "#define POSITIVEX", textureType, void 0, null, blockCompilation);
    this._face = 0;
  }
  /**
   * @internal
...
var FileFaceOrientation = class {
  constructor(name44, worldAxisForNormal, worldAxisForFileX, worldAxisForFileY) {
    this.name = name44;
...
Node.AddNodeConstructor("Light_Type_1", (name44, scene) => {
  return () => new DirectionalLight(name44, Vector3.Zero(), scene);
});
var DirectionalLight = class extends ShadowLight {
   ...
   */
  constructor(name44, direction, scene) {
    super(name44, scene);
...
Node.AddNodeConstructor("Light_Type_2", (name44, scene) => {
  return () => new SpotLight(name44, Vector3.Zero(), Vector3.Zero(), 0, 0, scene);
});
var SpotLight = class _SpotLight extends ShadowLight {
   ...
   */
  constructor(name44, position, direction, angle, exponent, scene) {
    super(name44, scene);
    this._innerAngle = 0;
...
var PBRMaterial = class _PBRMaterial extends PBRBaseMaterial {
   ...
   */
  clone(name44, cloneTexturesOnlyOnce = true, rootUrl = "") {
    const clone = SerializationHelper.Clone(() => new _PBRMaterial(name44, this.getScene()), this, { cloneTexturesOnlyOnce });
    clone.id = name44;
    clone.name = name44;
...
Node.AddNodeConstructor("Light_Type_0", (name44, scene) => {
  return () => new PointLight(name44, Vector3.Zero(), scene);
});
var PointLight = class extends ShadowLight {
   ...
   * @param scene The scene the lights belongs to
   */
  constructor(name44, position, scene) {
    super(name44, scene);
...
var MorphTarget = class _MorphTarget {
   ...
   */
  constructor(name44, influence = 0, scene = null) {
    this.name = name44;
    this.animations = [];
   ...
   * @returns a new MorphTarget
   */
  static FromMesh(mesh, name44, influence) {
    if (!name44) {
      name44 = mesh.name;
    }
    const result = new _MorphTarget(name44, influence, mesh.getScene());
...
var DracoCompression = class _DracoCompression {
   ...
   */
  async decodeMeshToGeometryAsync(name44, scene, data, attributes) {
    const meshData = await this._decodeMeshAsync(data, attributes);
    const geometry = new Geometry(name44, scene);
    if (meshData.indices) {
  ...
  /** @internal */
  async _decodeMeshToGeometryForGltfAsync(name44, scene, data, attributes, gltfNormalizedOverride) {
...
var FlowGraphConnection = class {
  constructor(name44, _connectionType, _ownerBlock) {
...
var FlowGraphDataConnection = class extends FlowGraphConnection {
   ...
   * @param richType
   */
  constructor(name44, connectionType, ownerBlock, richType) {
    super(name44, connectionType, ownerBlock);
...
var FlowGraphBlock = class {
   ...
   */
  registerDataInput(name44, richType) {
    const input = new FlowGraphDataConnection(name44, FlowGraphConnectionType.Input, this, richType);
   ...
   * @returns the created connection
   */
  registerDataOutput(name44, richType) {
    const output = new FlowGraphDataConnection(name44, FlowGraphConnectionType.Output, this, richType);
   ...
   */
  getDataInput(name44) {
    return this.dataInputs.find((i) => i.name === name44);
  }
  /**
   ...
   * @returns the connection if it exists, undefined otherwise
   */
  getDataOutput(name44) {
    return this.dataOutputs.find((i) => i.name === name44);
  }
  /**
...
var FlowGraphExecutionBlock = class extends FlowGraphBlock {
  ...
  _registerSignalInput(name44) {
    const input = new FlowGraphSignalConnection(name44, FlowGraphConnectionType.Input, this);
    this.signalInputs.push(input);
    return input;
  }
  _registerSignalOutput(name44) {
    const output = new FlowGraphSignalConnection(name44, FlowGraphConnectionType.Output, this);
   ...
   */
  getSignalInput(name44) {
    return this.signalInputs.find((input) => input.name === name44);
  }
  /**
   ...
   * @returns if the input exists, the input. Otherwise, undefined.
   */
  getSignalOutput(name44) {
    return this.signalOutputs.find((output) => output.name === name44);
  }
  /**
   * Serializes this block
...
var FlowGraphContext = class {
   ...
   */
  hasVariable(name44) {
    return name44 in this._userVariables;
  }
  /**
   ...
   */
  setVariable(name44, value) {
    this._userVariables[name44] = value;
  }
  /**
   ...
   */
  getVariable(name44) {
    return this._userVariables[name44];
  }
  /**
  ...
  _getUniqueIdPrefixedName(obj, name44) {
    return `${obj.uniqueId}_${name44}`;
  }
  /**
   ...
   * @param value
   */
  _setExecutionVariable(block, name44, value) {
    this._executionVariables[this._getUniqueIdPrefixedName(block, name44)] = value;
  }
  /**
   ...
   * @returns
   */
  _getExecutionVariable(block, name44, defaultValue) {
    if (this._hasExecutionVariable(block, name44)) {
      return this._executionVariables[this._getUniqueIdPrefixedName(block, name44)];
    } else {
      return defaultValue;
    }
  }
   ...
   * @param name
   */
  _deleteExecutionVariable(block, name44) {
    delete this._executionVariables[this._getUniqueIdPrefixedName(block, name44)];
  }
  /**
   ...
   * @returns
   */
  _hasExecutionVariable(block, name44) {
    return this._getUniqueIdPrefixedName(block, name44) in this._executionVariables;
  }
  /**
...
//# sourceMappingURL=chunk-SB4N7NJ2.js.map
